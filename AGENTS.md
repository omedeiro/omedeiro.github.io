...
- **Three pages carry JS**, deliberately and narrowly. `/habits` passes `wide` to `Base` for a 52rem column (five heatmaps do not fit in 42rem) and carries an `is:inline` script for the heatmap tooltip. `/maths/aperiodic-tiles` loads `public/maths/aperiodic-tiles.js` for the tiling builder and `/maths/euler-spiral` loads `public/maths/euler-spiral.js` for the curve viewer; neither can be done without a canvas, and both stay at 42rem. The `wide` column remains `/habits` alone. Every other page stays 42rem and JS-free — do not generalise any of these exceptions without asking.
- All three keep their CSS in `Base.astro`, scoped by a class prefix (`.habits-*`, `.tiling*`, `.euler*`), rather than adding a per-page stylesheet.
