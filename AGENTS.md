...
- **Four pages carry JS**, deliberately and narrowly. `/habits` passes `wide` to `Base` for a 52rem column (five heatmaps do not fit in 42rem) and carries an `is:inline` script for the heatmap tooltip. `/maths/aperiodic-tiles` loads `public/maths/aperiodic-tiles.js` for the tiling builder, `/maths/euler-spiral` loads `public/maths/euler-spiral.js` for the curve viewer, and `/maths/mandelbrot` loads `public/maths/mandelbrot.js` for the escape-time viewer; none can be done without a canvas, and all three stay at 42rem. The `wide` column remains `/habits` alone. Every other page stays 42rem and JS-free — do not generalise any of these exceptions without asking.
- All four keep their CSS in `Base.astro`, scoped by a class prefix (`.habits-*`, `.tiling*`, `.euler*`, `.mandel-*`), rather than adding a per-page stylesheet. Each viewer reads its palette back out of those custom properties with `getComputedStyle`, so light and dark are defined once, in the stylesheet.
